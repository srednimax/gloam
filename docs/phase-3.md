...
**The phone, read the same day, and it changes the shape of the bug.** [R8] had recorded that
`settings put system user_rotation` does nothing on HyperOS and that `wm user-rotation lock` is what
turns that display — the mechanism `apply_config` and `repin_rotation` both depend on. That is
withdrawn. With `MainActivity` in the foreground, `settings put` turns the phone perfectly:
`cur=2712x1220`, `ROTATION_90`. Read with the launcher in front, *neither* command rotates anything,
because the launcher is the portrait lock — which is what R8 actually met, and what this session's
first attempt at the question met before noticing. **The observation stands, the explanation does
not** — the same correction R7 took, one reading earlier.

**And the defect is a race, not a device family.** On the phone `am start -S` keeps the rotation,
because the restart is immediate and the launcher never settles; a bare `force-stop` that leaves the
app dead reverts it to portrait exactly as the emulator does. So the emulator is not uniquely broken
— it is merely slow enough to lose a race the phone usually wins, which is the worst version of this
bug rather than a milder one: landscape cells that are *intermittently* portrait, on a harness whose
whole job is evidence. That is also why the fix belongs after `wait_for_app()` as a *check* rather
than as a second write. It costs nothing when the race was won and repairs it when it was not, and
neither outcome needs the driver to know which device it is on.
